...
region_comuna.regiones.forEach(r => {
    region.innerHTML += `<option>${r.nombre}</option>`;
});

// Función mínima: actualizar comunas según la región seleccionada
region.onchange = () => {
    comuna.innerHTML = ''; // Limpiar comunas
    let reg = region_comuna.regiones[region.selectedIndex]; // Obtener región seleccionada
    reg.comunas.forEach(c => {
        comuna.innerHTML += `<option>${c.nombre}</option>`;
    });
};

// Cargar comunas de la primera región al inicio
region.onchange();