const data = {
  contact: ["Whatsapp","Telegram","X","Instagram","Tiktok","Otras"],
  tipo: ["Perro","Gato"],
  medida: ["Años","Meses"]
};
...
const poblarMEDIDA = () => {
  ...
  medidaSelect.innerHTML = '<option value="">Seleccione la medida</option>';

  for (const red of data.medida) {
      let option = document.createElement("option");
      option.value = red;
      option.text = red;
...
const poblarRegiones = () => {
    ...
    regionesSelect.innerHTML = '<option value="">Seleccione una región</option>';
...
const updateComunas = () => {
    ...
    if (selectedRegionNum) {
        const region = region_comuna.regiones.find(r => r.numero === selectedRegionNum);
        if (region) {
...
document.getElementById("select-redsocial").addEventListener("change", changeArguments);
